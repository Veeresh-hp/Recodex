...
const formatPrismaCertificate = (c: any) => ({
  ...
  verificationHash: (c.metadata as any)?.verificationHash || `0x${Array.from({ length: 40 }, () => Math.floor(Math.random() * 16).toString(16)).join("")}`,
...
router.post("/request", async (req: Request, res: Response) => {
  try {
    ...
    let certs = readCertificatesFile();
    const existingIdx = certs.findIndex(
      (c: any) => c.id === reqId || (c.userEmail === emailClean && c.projectName === finalProjectName && c.status === "Pending")
    );
    if (existingIdx >= 0) {
    ...
    try {
      let targetUser = null;
      if (userId) targetUser = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
      if (!targetUser && emailClean) targetUser = await prisma.user.findUnique({ where: { email: emailClean } }).catch(() => null);
      ...
      let targetProject = await prisma.project.findUnique({ where: { id: targetProjId } }).catch(() => null);
      if (!targetProject) {
        targetProject = await prisma.project.create({
          data: {
            id: targetProjId,
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const prismaResults = await prisma.certificate.findMany({
        where,
        orderBy: { createdAt: "desc" },
      });
    ...
    if (email) {
      ...
      allCerts = allCerts.filter((c: any) => (c.userEmail || "").toLowerCase().trim() === emailClean);
    } else if (userId) {
...
router.post("/", async (req: Request, res: Response) => {
  try {
    ...
    const existingIndex = certs.findIndex(
      (c: any) => c.id === certId || (emailClean && c.userEmail === emailClean && c.projectName === projectName)
    );
    if (existingIndex >= 0) {
    ...
    try {
      let targetUser = null;
      if (cert.userId) {
        targetUser = await prisma.user.findUnique({ where: { id: cert.userId } }).catch(() => null);
      }
      if (!targetUser && emailClean) {
        targetUser = await prisma.user.findUnique({ where: { email: emailClean } }).catch(() => null);
      }
      if (!targetUser && emailClean) {
        targetUser = await prisma.user.create({
          data: {
      ...
      if (targetUser && targetProject) {
        ...
        }).catch((err: any) => console.warn("Prisma certificate save note:", err));
      }
    } catch (prismaErr) {

...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        let projects = await db_1.default.project.findMany({
            where: filters,
            select: {
                id: true,
                title: true,
 ...
 */
router.get("/my-projects", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (isAdmin) {
            ...
            projects = await db_1.default.project.findMany({
                where: {
                    OR: [
 ...
 */
router.get("/my-assignments", auth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: "Unauthorized." });
        }
        const assignments = await db_1.default.projectAssignment.findMany({
            where: { userId },
            include: {
                project: true,
            },
 ...
 */
router.get("/:id/assignments", auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    try {
        const assignments = await db_1.default.projectAssignment.findMany({
            where: { projectId: id },
        ...
        const assignmentsWithCerts = await Promise.all(assignments.map(async (asgn) => {
 ...
 */
router.post("/:id/assign", auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const assignments = await certificateService_1.ProjectAssignmentService.assignUsersToProject(id, userIds, adminId);
 ...
 */
router.post("/assignments/:assignmentId/complete", auth_1.requireAuth, async (req, res) => {
    const { assignmentId } = req.params;
    const { completionDate, notes, certificateAction, scheduledDays, score } = req.body;
    try {
        const adminId = req.user?.id || "ADMIN";
        const result = await certificateService_1.ProjectAssignmentService.markAssignmentCompleted(assignmentId, {
 ...
 */
router.post("/assignments/:assignmentId/issue-certificate", auth_1.requireAuth, async (req, res) => {
    const { assignmentId } = req.params;
    const { score, grade, reason } = req.body;
    try {
        const adminId = req.user?.id || "ADMIN";
        const cert = await certificateService_1.ProjectAssignmentService.adminIssueCertificateToUser(assignmentId, {
            score: score ? parseFloat(score) : 100.0,
            grade,
            reason,
        }, adminId);
...
router.post("/:id/submit", auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const project = await db_1.default.project.findUnique({ where: { id } });
        if (!project) {
            return res.status(404).json({ error: "Project not found." });
        }
        // Upsert project submission
        const submission = await db_1.default.projectSubmission.create({
            data: {
                projectId: id,
                userId,
                title: project.title,
        ...
        await db_1.default.projectAssignment.upsert({
            where: {
...
router.post("/", auth_1.requireAuth, upload.single("image"), async (req, res) => {
    try {
        const { id, title, description, longDescription, status, category, tags, files, certificateEnabled, certificateName, completionMethod, scheduledCompletionAt, automaticIssuance, issuanceDelayDays, assignedUserIds, assignedEmail, assignedUserId, assignedUserName, repoUrl, liveUrl, progress, startDate, expectedDate, contractId, isPrivate, milestones, deliverables, } = req.body;
        ...
        if (cleanAssignedEmail && !cleanAssignedUserId) {
            try {
                const foundUser = await db_1.default.user.findUnique({ where: { email: cleanAssignedEmail } });
                if (foundUser) {
                    cleanAssignedUserId = foundUser.id;
                    if (!cleanAssignedUserName)
                        cleanAssignedUserName = foundUser.name;
                }
            }
            catch (e) { }
        }
        ...
        if (cleanAssignedUserId && !cleanAssignedEmail) {
            try {
                const foundUser = await db_1.default.user.findUnique({ where: { id: cleanAssignedUserId } });
                if (foundUser) {
        ...
        if (cleanAssignedUserId) {
            try {
                await certificateService_1.ProjectAssignmentService.assignUsersToProject(project.id, [cleanAssignedUserId], req.user?.id || "ADMIN");
            }
            catch (err) {
        ...
        if (assignedUserIds) {
            ...
            if (Array.isArray(userIdsToAssign) && userIdsToAssign.length > 0) {
                await certificateService_1.ProjectAssignmentService.assignUsersToProject(project.id, userIdsToAssign, req.user?.id || "ADMIN");
            }
        }
...
router.put("/:id", auth_1.requireAuth, upload.single("image"), async (req, res) => {
    ...
    try {
        ...
        const { title, description, longDescription, status, category, tags, files, certificateEnabled, certificateName, completionMethod, scheduledCompletionAt, automaticIssuance, issuanceDelayDays, } = req.body;
        const updateData = {};
        if (title !== undefined)
        ...
        if (completionMethod !== undefined)
            updateData.completionMethod = completionMethod;
...
router.patch("/:id", auth_1.requireAuth, async (req, res) => {
    const { id } = req.params;
    const { status, progress, expectedDate, startDate, assignedEmail, assignedUserId, assignedUserName, repoUrl, liveUrl, title, category, isPrivate, } = req.body;
    try {
        const existing = await db_1.default.project.findUnique({ where: { id } });
        if (!existing) {
            return res.status(404).json({ error: "Project not found." });
        }
        const data = {};
        ...
        if (assignedEmail !== undefined)
            data.assignedEmail = assignedEmail ? String(assignedEmail).toLowerCase().trim() : null;
        ...
        if (status !== undefined || progress !== undefined) {
            try {
                await db_1.default.projectAssignment.updateMany({
                    where: { projectId: id },
                    data: {

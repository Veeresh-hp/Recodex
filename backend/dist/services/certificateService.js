...
exports.ProjectAssignmentService = exports.CertificateService = void 0;
const client_1 = require("@prisma/client");
const certificateId_1 = require("../utils/certificateId");
...
class CertificateService {
    ...
    static async issueCertificate(params) {
        const { userId, projectId, projectAssignmentId, submissionId, issuanceMethod = "AUTOMATIC", issuedBy = "SYSTEM", overrideReason, status = "ISSUED", } = params;
        ...
        let certificateId = (0, certificateId_1.generateCertificateId)(new Date(issueDate).getFullYear());
        // Ensure collision avoidance in database
        let collisionCheck = await prisma.certificate.findUnique({ where: { certificateId } });
        while (collisionCheck) {
            certificateId = (0, certificateId_1.generateCertificateId)(new Date(issueDate).getFullYear());
        ...
        if (status === "SCHEDULED" || status === "PENDING") {
            const qrCodeUrl = await (0, qrGenerator_1.generateCertificateQrCodeDataUrl)(certificateId);
        ...
        let pdfBuffer;
        try {
            pdfBuffer = await (0, pdfGenerator_1.generateCertificatePdf)({
                certificateId,
                recipientName,
                recipientEmail,
                projectTitle,
        ...
        let pdfUrl = "";
        try {
            const uploadRes = await (0, cloudinary_1.uploadToCloudinary)(pdfBuffer, "recodex_certificates");
     ...
     */
    static async finalizeCertificateIssuance(certIdOrCertificateId, options) {
     ...
     */
    static async revokeCertificate(certificateId, reason, adminId) {
     ...
     */
    static async restoreCertificate(certificateId, adminId) {
        const cert = await prisma.certificate.findUnique({
            where: { certificateId },
        });
        if (!cert) {
     ...
     */
    static async regenerateCertificate(certificateId, adminId) {
        const cert = await prisma.certificate.findUnique({
            where: { certificateId },
        });
        if (!cert) {
...
class ProjectAssignmentService {
     ...
     */
    static async assignUsersToProject(projectId, userIds, adminId = "ADMIN") {
        const project = await prisma.project.findUnique({ where: { id: projectId } });
        if (!project) {
     ...
     */
    static async markAssignmentCompleted(assignmentId, options, adminId) {
     ...
     */
    static async adminIssueCertificateToUser(assignmentId, options, adminId) {
        const assignment = await prisma.projectAssignment.findUnique({
            where: { id: assignmentId },
            include: { project: true, user: true },
        });
